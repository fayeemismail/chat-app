...
const Notifications = () => {
    ...
    useEffect(() => {
        const fetchNotifications = async () => {
            try {
                const response = await axios.get(`/api/users/notifications/?userId=${userId}`);
                setNotifications(response.data);

                // Fetch following status
                const followRes = await axios.get(`/api/users/following/?userId=${userId}`);
    ...
    const followBack = async (id, isPrivate) => {
        try {
            ...
            await axios.post('/api/users/sendFollow', { targetUserId: id, currentUserId: userId });
    ...
    const acceptRequest = async(targetId, notificationId) => {
        try {
            ...
            await axios.post('/api/users/accept_request', {userId: userId, targetId: targetId, notificationId: notificationId});
    ...
    return (
        <div className="min-h-screen font-serif" style={{ backgroundColor: colors.bg }}>
                            ...
                            >
                                <div>
                                    <p className="mb-1" style={{ color: colors.text }}>
                                         {notification.type === "follow_request" ? "sent you a follow request" : notification.message}
                                    </p>
                                ...
                                <div className="flex space-x-3">
                                    {notification.type == 'follow_request' ? (
                                        ...
                                        onClick={() => acceptRequest(notification.sender._id, notification._id)}
                                        style={buttonStyle}
                                        onMouseOver={(e) => {
                                    ...
                                    ) : (notification.sender.followers.includes(userId) && notification.sender.following.includes(userId)) ? 
                                    (
                                        <button 
                                            className="hover:opacity-90 transition-opacity"
                                            style={buttonStyle}
                                        ...
                                        <button 
                                            className="hover:opacity-90 transition-opacity"
                                            onClick={() => followBack(notification.sender._id, notification.sender.isPrivate)}
                                            style={buttonStyle}
                                            onMouseOver={(e) => {
                                                Object.assign(e.currentTarget.style, buttonHoverStyle);
                                            }}
