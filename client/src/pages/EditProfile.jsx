...
const EditProfile = () => {
    ...
    const [user, setUser] = useState({ name: '', bio: '', isPrivate: false, profilePhoto: '' });
        ...
        const fetchUserData = async () => {
            try {
                ...
                setFormData({
                    ...
                    password: '',
                    profilePhoto: response.data.profilePhoto, // Correct field name
                });
            } catch (error) {
    ...
    // Ensure formData updates when user state changes
    useEffect(() => {
        setFormData((prev) => ({
            ...prev,
            name: user.name,
            ...
            profilePhoto: user.profilePhoto, // Corrected field
        }));
    }, [user]);
    ...
    const handleFileUpload = async (file) => {
        ...
        try {
            ...
            setFormData((prev) => ({
                ...prev,
                profilePhoto: response.data.secure_url // Corrected field
            }));
        } catch (error) {
                ...
                <div className={styles.formContainer}>
                    <form onSubmit={handleSubmit} className="space-y-6">
                        ...
                        <div>
                            ...
                            <textarea name="bio" value={user.bio} onChange={handleInputChange} rows="3" className={styles.input}></textarea>
                        </div>
                        ...
                        <div className="flex items-center justify-between">
                            ...
                            <div className="relative">
                                <input type="checkbox" checked={showPasswordFields} onChange={() => setShowPasswordFields((prev) => !prev)} className="sr-only" id="toggle-password"
                                />
                                <label
                        ...
                        {showPasswordFields && (
                            <>
                                <div>
                                    ...
                                    <input
                                        type="password" name="password" value={formData.password} onChange={handlePasswordChange} className={styles.input} required={showPasswordFields}
                                    />
                                </div>
                                <div>
                                    <label className={styles.label}>Confirm Password</label>
                                    <input type="password" name="confirmPassword" value={formData.confirmPassword} onChange={handlePasswordChange} className={styles.input} required={showPasswordFields}
                                    />
                                </div>
                            </>
                        )}
                        ...
                        <div className="flex items-center justify-between">
                            ...
                            <div className="relative">
                                <input type="checkbox" name="isPrivate" checked={user.isPrivate} onChange={handleInputChange} className="sr-only" id="toggle-private"
                                />
