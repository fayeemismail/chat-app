    ...
    if (!user) {
        return (
            <div className={`flex justify-center items-center min-h-screen font-serif ${
                darkMode ? 'bg-[#1A1A1A] text-[#A9A295]' : 'bg-[#F5F3EE] text-[#6D6459]'
            }`}>
                <div className="italic tracking-wide">Loading profile information...</div>
            </div>
        );
    }
                ...
                <div className={`rounded-sm shadow-sm p-6 ${
                            ...
                            {user.profilePicture ? (
                                <img
                                    ...
                                    className={`w-24 h-24 rounded-full object-cover relative z-10 ${
                                        darkMode ? 'border-2 border-[#C9AD6A]' : 'border-2 border-[#9B8759]'
                                    }`}
                                />
                            ) : (
                                <div className={`w-24 h-24 rounded-full flex items-center justify-center text-2xl font-medium relative z-10 ${
                                    darkMode 
                    ...
                    <div className="grid grid-cols-2 gap-3 mb-6">
                        <div className={`p-4 text-center ${
                            darkMode ? 'bg-[#333333] border border-[#333333]' : 'bg-[#F9F7F2] border border-[#E2DFD6]'
                        }`}>
                            <div className="text-2xl font-medium">{user.followers.length || 0}</div>
                        ...
                        <div className={`p-4 text-center ${
                            ...
                            <div className="text-2xl font-medium">{user.following.length || 0}</div>
                            <div className={`text-xs uppercase tracking-wider ${
                                darkMode ? 'text-[#A9A295]' : 'text-[#6D6459]'
                            }`}>Following</div>
                        </div>
                    ...
                    <div className="flex justify-center mt-4">
                        <button className={`px-6 py-2 text-xs uppercase tracking-wider font-medium transition-colors 
